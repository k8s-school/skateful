...
do
    ...
    "$DIR"/yaml-builder.py -n "$NS" -P "$DATA_PATH" -p "$PVC_NAME" -H $host -o "$YAML_OUT_DIR" -i "$INSTANCE"
    COUNT=$((COUNT+1))
done
...
DATA_PATH="$DATA_DIR/ingest"
"$DIR"/yaml-builder.py -n "$NS" -P "$DATA_PATH" -p "$PVC_NAME" -H $INGEST_DB_HOST -o "$YAML_OUT_DIR" -i "$INSTANCE"
